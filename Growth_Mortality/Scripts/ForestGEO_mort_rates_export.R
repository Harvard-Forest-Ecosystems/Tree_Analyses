...
dat14small <- filter(dat_14, dbh <=10)
dat19small <- filter(dat_19, dbh <=10)

# need to filter out the swamp for more direct comparison
summary(dat14small)
summary(dat19small)
...
dat2 <- merge(dat_14, dat_19, by = c('tree.id','stem.id'))
write_csv(dat2, 'GitHub/Dendrometer/data/forestgeo_data/mortality_indiv_all')



# make dataset of dead trees
dat_19_dead <- filter(dat_19, df.status == 'stem dead')

dead_dat <- inner_join(dat_14, dat_19_dead, by =  c('tree.id','stem.id'))

# export
write_csv(dead_dat, 'GitHub/Dendrometer/data/forestgeo_data/mortality_indiv_only_dead')


### ASK JACKIE: should I include stems that are gone in the mortality rate
# or just ones that are marked dead
...
dead_mort <- dead_trees %>%
  group_by(quadrat.x) %>%
  summarize(kgc_mort = sum(kgc.x, na.rm = TRUE),
            ba_mort = sum(basal_area.x, na.rm = TRUE),
            n_dead = n())
...
mortal %>% ggplot() + 
  geom_tile(aes(x=quad_row, y=quad_col, fill= ba_mort))+
  theme_minimal()+
  ylab('Quadrat Column')+
  xlab('Quadrat Row')+
  labs(fill = "Mortality Rate (BA Loss)")

# export
write_csv(mortal, 'GitHub/Dendrometer/data/forestgeo_data/mortality_quadrat_only_dead')
